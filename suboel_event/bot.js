...
bot.on("message", function(message) {
  ...
  // set variable to all args
	...
	switch (args[0].toLowerCase()) {
		...
		// command test
		case "test":
			message.member.guild.createChannel("test1", "text", [{
				id: message.member.guild.roles.find("name", "bot").id,
				deny: 1024,
				allow: 1,
				type: 'role'
			}]).then(function() {
				message.member.guild.setChannelPosition(message.member.guild.channels.find("name", "test1").id, 1);
			});
			break;
